...
try{
  ...
  const voteInputMode=await evaluate(`(()=>{
    ...
    const rawPercentValues=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),rawPercentTotal=rawPercentValues.reduce((sum,value)=>sum+value,0),rawPercentStayedClean=!current().dirty;
    document.querySelector('[data-calc-vote-mode="count"]').click();const rawConvertedCounts=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value));document.querySelector('[data-calc-vote-mode="percent"]').click();
    ...
    const normalizedAfterCalculate=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),normalizedAfterCalculateTotal=Number(normalizedAfterCalculate.reduce((sum,value)=>sum+value,0).toFixed(6));
    ...
    const fractionalHighlight={highlighted,invalid,glow,correctedCleared,otherStayed,allCleared:!document.querySelector('.p-votes.is-fractional-vote'),warning:document.getElementById('error').textContent.trim()};
