...
try{
  ...
  const voteInputMode=await evaluate(`(()=>{
    ...
    const defaultPair=document.querySelector('.calc-vote-pair'),defaultPairRect=defaultPair.getBoundingClientRect(),defaultVoteCellRect=defaultPair.closest('td').getBoundingClientRect(),deleteButtonRect=document.querySelector('#inputRows tr:first-child .p-del').getBoundingClientRect(),deleteCellRect=document.querySelector('#inputRows tr:first-child .p-del').closest('td').getBoundingClientRect(),defaultFields=[...document.querySelectorAll('#inputRows tr:first-child .calc-vote-field')],headingLabel=document.querySelector('.calc-vote-heading-label').getBoundingClientRect(),defaultToggle=document.getElementById('calcVoteModeToggle').getBoundingClientRect(),defaultToggleButtons=[...document.querySelectorAll('[data-calc-vote-mode]')].map(button=>button.getBoundingClientRect().width),defaultMode={countPressed:document.querySelector('[data-calc-vote-mode="count"]').getAttribute('aria-pressed'),percentPressed:document.querySelector('[data-calc-vote-mode="percent"]').getAttribute('aria-pressed'),countLabel:document.querySelector('[data-calc-vote-mode="count"]').textContent.trim(),percentLabel:document.querySelector('[data-calc-vote-mode="percent"]').textContent.trim(),summaryLabel:document.getElementById('cardVotesLabel').textContent.trim(),step:document.querySelector('.p-votes').step,inputMode:document.querySelector('.p-votes').inputMode,fields:defaultFields.length,fieldWidths:defaultFields.map(field=>field.getBoundingClientRect().width),pairWidth:defaultPairRect.width,voteCellWidth:defaultVoteCellRect.width,rightGap:defaultVoteCellRect.right-defaultPairRect.right,deleteGap:deleteButtonRect.left-defaultPairRect.right,deleteCellWidth:deleteCellRect.width,deleteButtonWidth:deleteButtonRect.width,equivalentDisabled:document.querySelector('.p-votes-equivalent').disabled,equivalentBackground:getComputedStyle(document.querySelector('.p-votes-equivalent')).backgroundColor,equivalents:[...document.querySelectorAll('.p-votes-equivalent')].map(input=>Number(input.value)),toggleWidth:defaultToggle.width,toggleButtonWidths:defaultToggleButtons,headingOneLine:Math.abs((headingLabel.top+headingLabel.height/2)-(defaultToggle.top+defaultToggle.height/2))<1};
    ...
    current().parties=[20,50,29.9].map((votes,index)=>({...parties[index],votes,order:index}));current().dirty=false;renderInputs();
    const autoCorrected=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),autoCorrectedTotal=Number(autoCorrected.reduce((sum,value)=>sum+value,0).toFixed(6)),autoCorrectedMarkedDirty=current().dirty,editInputs=[...document.querySelectorAll('.p-votes')];
    editInputs[2].value='40';editInputs[2].dispatchEvent(new Event('input',{bubbles:true}));
    const proportionallyAdjusted=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),adjustedTotal=Number(proportionallyAdjusted.reduce((sum,value)=>sum+value,0).toFixed(6)),adjustedRatio=proportionallyAdjusted[0]/proportionallyAdjusted[1];
    editInputs[2].value='';editInputs[2].dispatchEvent(new Event('input',{bubbles:true}));
    const clearedValues=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),clearedTotal=Number(clearedValues.reduce((sum,value)=>sum+value,0).toFixed(6)),clearedStayedBlank=editInputs[2].value==='';
    current().parties=current().parties.slice(0,2);renderInputs();const afterDelete=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),afterDeleteTotal=Number(afterDelete.reduce((sum,value)=>sum+value,0).toFixed(6));
  ...
  if(voteInputMode.defaultMode.countPressed!=='true'||voteInputMode.defaultMode.percentPressed!=='false'||voteInputMode.defaultMode.countLabel!=='Antal röster'||voteInputMode.defaultMode.percentLabel!=='Röster %'||voteInputMode.defaultMode.summaryLabel!=='Totalt antal röster'||voteInputMode.defaultMode.step!=='1'||voteInputMode.defaultMode.inputMode!=='numeric'||voteInputMode.defaultMode.fields!==2||voteInputMode.defaultMode.pairWidth<360||voteInputMode.defaultMode.rightGap>1.5||voteInputMode.defaultMode.deleteGap>7.5||Math.abs(voteInputMode.defaultMode.fieldWidths[0]-voteInputMode.defaultMode.fieldWidths[1])>.1||!voteInputMode.defaultMode.equivalentDisabled||voteInputMode.defaultMode.equivalentBackground!=='rgb(229, 231, 232)'||JSON.stringify(voteInputMode.defaultMode.equivalents)!==JSON.stringify([36.5,28.7,17.1,10.5,7.2])||!voteInputMode.defaultMode.headingOneLine||voteInputMode.defaultMode.toggleWidth>190||Math.abs(voteInputMode.defaultMode.toggleButtonWidths[0]-voteInputMode.defaultMode.toggleButtonWidths[1])>.1||voteInputMode.percentMode.countPressed!=='false'||voteInputMode.percentMode.percentPressed!=='true'||voteInputMode.percentMode.summaryLabel!=='Total %'||voteInputMode.percentMode.step!=='0.01'||voteInputMode.percentMode.max!=='100'||voteInputMode.percentMode.inputMode!=='decimal'||voteInputMode.percentMode.unit!=='%'||JSON.stringify(voteInputMode.percentMode.converted)!==JSON.stringify([36.5,28.7,17.1,10.5,7.2])||JSON.stringify(voteInputMode.percentMode.roundTripCounts)!==JSON.stringify([365,287,171,105,72])||voteInputMode.percentMode.fields!==1||voteInputMode.percentMode.hasCountField||voteInputMode.percentMode.pairColumns.split(' ').length!==1||voteInputMode.percentMode.rightGap>1.5||voteInputMode.percentMode.deleteGap>7.5||Math.abs(voteInputMode.percentMode.activeWidth-voteInputMode.percentMode.pairWidth)>.1||Math.abs(voteInputMode.defaultMode.pairWidth-voteInputMode.percentMode.pairWidth)>.1||Math.abs(voteInputMode.defaultMode.toggleWidth-voteInputMode.percentMode.percentToggleWidth)>.1||voteInputMode.percentMode.autoCorrectedTotal!==100||!voteInputMode.percentMode.autoCorrectedMarkedDirty||voteInputMode.percentMode.adjustedTotal!==100||Math.abs(voteInputMode.percentMode.proportionallyAdjusted[2]-40)>.000001||Math.abs(voteInputMode.percentMode.adjustedRatio-.4)>.000001||voteInputMode.percentMode.clearedTotal!==100||!voteInputMode.percentMode.clearedStayedBlank||voteInputMode.percentMode.afterDeleteTotal!==100||!voteInputMode.sameAllocation||!voteInputMode.sameDhondtAllocation||voteInputMode.total!==100||voteInputMode.firstQuotient<=0||voteInputMode.firstQuotient>=100||voteInputMode.cleanMode!=='percent'||voteInputMode.cleanBasis!==1000||voteInputMode.restoredMode!=='percent'||voteInputMode.restoredBasis!==1000||!voteInputMode.countDecimalRejected||!voteInputMode.percentOverHundredRejected||!voteInputMode.exportSafe||voteInputMode.dirty)failures.push('vote input mode');
