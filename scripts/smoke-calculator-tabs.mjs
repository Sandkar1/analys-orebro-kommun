...
try{
  ...
  const voteInputMode=await evaluate(`(()=>{
    ...
    const defaultFields=[...document.querySelectorAll('#inputRows tr:first-child .calc-vote-field')],headingLabel=document.querySelector('.calc-vote-heading-label').getBoundingClientRect(),defaultToggle=document.getElementById('calcVoteModeToggle').getBoundingClientRect(),defaultToggleButtons=[...document.querySelectorAll('[data-calc-vote-mode]')].map(button=>button.getBoundingClientRect().width),defaultMode={countPressed:document.querySelector('[data-calc-vote-mode="count"]').getAttribute('aria-pressed'),percentPressed:document.querySelector('[data-calc-vote-mode="percent"]').getAttribute('aria-pressed'),countLabel:document.querySelector('[data-calc-vote-mode="count"]').textContent.trim(),percentLabel:document.querySelector('[data-calc-vote-mode="percent"]').textContent.trim(),summaryLabel:document.getElementById('cardVotesLabel').textContent.trim(),step:document.querySelector('.p-votes').step,inputMode:document.querySelector('.p-votes').inputMode,fields:defaultFields.length,fieldWidths:defaultFields.map(field=>field.getBoundingClientRect().width),pairWidth:document.querySelector('.calc-vote-pair').getBoundingClientRect().width,equivalentDisabled:document.querySelector('.p-votes-equivalent').disabled,equivalentBackground:getComputedStyle(document.querySelector('.p-votes-equivalent')).backgroundColor,equivalents:[...document.querySelectorAll('.p-votes-equivalent')].map(input=>Number(input.value)),toggleWidth:defaultToggle.width,toggleButtonWidths:defaultToggleButtons,headingOneLine:Math.abs((headingLabel.top+headingLabel.height/2)-(defaultToggle.top+defaultToggle.height/2))<1};
    document.querySelector('[data-calc-vote-mode="percent"]').click();
    const converted=[...document.querySelectorAll('.p-votes')].map(input=>Number(input.value)),percentPair=document.querySelector('.calc-vote-pair'),pairColumns=getComputedStyle(percentPair).gridTemplateColumns,percentFields=[...document.querySelectorAll('#inputRows tr:first-child .calc-vote-field')],activeWidth=percentFields[0].getBoundingClientRect().width,pairWidth=percentPair.getBoundingClientRect().width,percentToggleWidth=document.getElementById('calcVoteModeToggle').getBoundingClientRect().width;
    ...
    const percentMode={countPressed:document.querySelector('[data-calc-vote-mode="count"]').getAttribute('aria-pressed'),percentPressed:document.querySelector('[data-calc-vote-mode="percent"]').getAttribute('aria-pressed'),summaryLabel:document.getElementById('cardVotesLabel').textContent.trim(),step:inputs[0].step,max:inputs[0].max,inputMode:inputs[0].inputMode,unit:document.querySelector('.calc-vote-input-unit')?.textContent||'',values:inputs.map(input=>Number(input.value)),converted,roundTripCounts,fields:percentFields.length,hasCountField:!!document.querySelector('[aria-label^="Antal röster"]'),pairColumns,activeWidth,pairWidth,percentToggleWidth,autoCorrected,autoCorrectedTotal,autoCorrectedMarkedDirty,proportionallyAdjusted,adjustedTotal,adjustedRatio,clearedTotal,clearedStayedBlank,afterDelete,afterDeleteTotal};
