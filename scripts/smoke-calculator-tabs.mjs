...
try{
  ...
  const voteInputMode=await evaluate(`(()=>{
    ...
    const percentMode={countPressed:document.querySelector('[data-calc-vote-mode="count"]').getAttribute('aria-pressed'),percentPressed:document.querySelector('[data-calc-vote-mode="percent"]').getAttribute('aria-pressed'),step:inputs[0].step,max:inputs[0].max,inputMode:inputs[0].inputMode,unit:document.querySelector('.calc-vote-input-unit')?.textContent||'',values:inputs.map(input=>Number(input.value)),converted,roundTripCounts,fields:percentFields.length,hasCountField:!!document.querySelector('[aria-label^="Antal röster"]'),pairColumns,activeWidth,pairWidth,percentToggleWidth,autoCorrected,autoCorrectedTotal,autoCorrectedMarkedDirty,proportionallyAdjusted,adjustedTotal,adjustedRatio,clearedTotal,clearedStayedBlank,afterDelete,afterDeleteTotal};
  ...
  const mobileAfter=await evaluate(`(()=>{document.querySelector('[data-calc-vote-mode="percent"]').click();return {ids:tabs.map(tab=>tab.id),activeId:current().id,activeTab,templateFirst:isTemplateTab(tabs[0]),domIds:[...document.querySelectorAll('.tab-wrap')].map(node=>Number(node.dataset.tabId)),pageFits:document.documentElement.scrollWidth===document.documentElement.clientWidth,settingsColumns:getComputedStyle(document.getElementById('calcSettings')).gridTemplateColumns,percentFields:document.querySelectorAll('#inputRows tr:first-child .calc-vote-field').length,percentHasCount:!!document.querySelector('#inputRows [aria-label^="Antal röster"]'),pointerEvents:window.__calcPointerEvents};})()`);
  ...
  if(mobileAfter.ids[0]!==mobileBefore.ids[0]||mobileAfter.ids[1]!==mobileBefore.ids[2]||mobileAfter.activeId!==mobileBefore.activeId||mobileAfter.activeTab!==1||mobileAfter.ids.join(',')!==mobileAfter.domIds.join(',')||!mobileAfter.templateFirst||!mobileAfter.pageFits||mobileAfter.settingsColumns.split(' ').length!==1||mobileAfter.percentFields!==1||mobileAfter.percentHasCount)failures.push('mobile drag');
