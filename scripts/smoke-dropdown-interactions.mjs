...
try{
  ...
  const rawFilterClearRestore=await evaluate(`(async()=>{
    ...
    const fullCount=rawProgressiveSearchStateFinal.matches.length,baselineKey=rawResultMemoKeyFinal(),year=rawFilterOptionsFinal()?.years?.[0]||'';
    ...
    renderRawFilterLocks();rawScheduleProgressiveFinal();
    return {year,fullCount,filteredCount,firstFilterSettled,baselinePinned:rawBaselineResultMemoFinal.has(baselineKey),firstRestore,secondFilter,secondRestore};
  })()`);
  ...
  const rawPartyOrdering=await evaluate(`(async()=>{
    ...
    for(let frame=0;frame<7;frame++)await new Promise(resolve=>requestAnimationFrame(resolve));
    return {ok:descending&&JSON.stringify(actual)===JSON.stringify(expected),options:actual.length,actual:actual.slice(0,12),votes:votes.slice(0,12),expected:expected.slice(0,12)};
  })()`);
  ...
  await evaluate(`(()=>{Object.keys(rawFilterLocks).forEach(key=>{rawFilterLocks[key]=[];});rawFilterLocks.rawYear=['2010','2014'];rawFilterLocks.rawMunicipality=['Örebro'];rawFilterLocks.rawParty=['S','M'];buildRawFilters();rawScheduleProgressiveFinal();return true})()`);
  ...
  const facetedFilterSemantics=await evaluate(`(()=>{
    const years=[...rawFilterLocks.rawYear],municipalities=[...rawFilterLocks.rawMunicipality],parties=[...rawFilterLocks.rawParty],query='';
    const filters={years,elections:[],counties:[],municipalities,parties};
    const expected=rawRows.filter(row=>rawProgressivePredicateFinal(row,filters,query)).length;
    const rows=rawProgressiveSearchStateFinal.matches;
    return {years,municipalities,parties,matches:rows.length,expected,
      everyYearAllowed:rows.every(row=>years.includes(String(rawComparable(row,'year')))),
      everyMunicipalityAllowed:rows.every(row=>municipalities.includes(rawComparable(row,'municipality_name'))),
      everyPartyAllowed:rows.every(row=>parties.includes(rawComparable(row,'party_standard'))),
      bothYearsPresent:new Set(rows.map(row=>String(rawComparable(row,'year')))).size===years.length,
      bothPartiesPresent:new Set(rows.map(row=>rawComparable(row,'party_standard'))).size===parties.length,
      sameCategoryOr:filterValueSetsMatch([{values:['2010','2014'],matches:value=>value==='2014'}]),
      crossCategoryAndRejects:!filterValueSetsMatch([{values:['2014'],matches:value=>value==='2014'},{values:['S'],matches:value=>value==='M'}]),
      empty:filterValueSetsMatch([]),
      controlsRemoved:!document.querySelector('[data-filter-mode-toggle],.filter-mode-toggle')};
  })()`);
  await evaluate(`(()=>{Object.keys(rawFilterLocks).forEach(key=>{rawFilterLocks[key]=[];});rawFilterLocks.rawYear=['2010'];rawFilterLocks.rawParty=['S'];buildRawFilters();rawScheduleProgressiveFinal();return true})()`);
  await waitFor(`rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal()`,500);
  ...
  await waitFor(`document.readyState==='complete'&&typeof currentTopView==='function'&&!!rawDataPromise&&!rawReady&&JSON.stringify(rawFilterLocks.rawYear)===${JSON.stringify(reloadExpected.raw)}&&JSON.stringify(rawFilterLocks.rawParty)===${JSON.stringify(reloadExpected.rawParty)}&&rawProgressiveSearchStateFinal?.index>0`,500);
  ...
  await waitFor(`currentTopView()===${JSON.stringify(reloadExpected.top)}&&!urlHashApplying&&JSON.stringify(decisionFilterLocks.decisionOrgan)===${JSON.stringify(reloadExpected.decision)}&&JSON.stringify(decisionActivityFilters.type)===${JSON.stringify(reloadExpected.activity)}&&document.getElementById('decisionActivityType').options.length>1&&JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])]))===${JSON.stringify(reloadExpected.calculator)}`,500);
  const reload=await evaluate(`({mode:location.protocol==='file:'?'file':'http',hash:location.hash,top:currentTopView(),raw:JSON.stringify(rawFilterLocks.rawYear),decision:JSON.stringify(decisionFilterLocks.decisionOrgan),activity:JSON.stringify(decisionActivityFilters.type),andOrControls:document.querySelectorAll('[data-filter-mode-toggle],.filter-mode-toggle').length,calculator:JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])])),activityOptions:document.getElementById('decisionActivityType').options.length,promptDisabled:document.getElementById('decisionActivityType').options[0]?.disabled??true})`);
  ...
  if(!facetedFilterSemantics.sameCategoryOr||!facetedFilterSemantics.crossCategoryAndRejects||!facetedFilterSemantics.empty||!facetedFilterSemantics.controlsRemoved||facetedFilterSemantics.years.length!==2||facetedFilterSemantics.municipalities.length!==1||facetedFilterSemantics.parties.length!==2||facetedFilterSemantics.matches<=0||facetedFilterSemantics.matches!==facetedFilterSemantics.expected||!facetedFilterSemantics.everyYearAllowed||!facetedFilterSemantics.everyMunicipalityAllowed||!facetedFilterSemantics.everyPartyAllowed||!facetedFilterSemantics.bothYearsPresent||!facetedFilterSemantics.bothPartiesPresent)process.exitCode=1;
  ...
  if(reload.andOrControls!==0)process.exitCode=1;
}finally{
