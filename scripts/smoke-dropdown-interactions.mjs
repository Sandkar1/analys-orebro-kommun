...
try{
  ...
  const calculatorTemplate=await evaluate(`(()=>{
    const before={tabs:tabs.length,activeTab,role:tabs[0]?.role,title:tabs[0]?.title,closeButtons:document.querySelectorAll('.calc-tab-close').length,settingsHidden:document.getElementById('calcSettings').hidden,summaryHidden:document.getElementById('calcSummary').hidden,actionsRemoved:document.getElementById('calcActions').hidden&&getComputedStyle(document.getElementById('calcActions')).display==='none',templateVisible:!document.getElementById('calcTemplateIntro').hidden,addBelowTable:document.querySelector('.mobile-table-scroll')?.nextElementSibling?.contains(document.getElementById('add'))||false};
    closeCalcTab(0);
