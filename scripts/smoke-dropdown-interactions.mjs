...
try{
  ...
  await waitFor(`document.readyState==='complete'&&typeof currentTopView==='function'&&currentTopView()===${JSON.stringify(reloadExpected.top)}&&JSON.stringify(rawFilterLocks.rawYear)===${JSON.stringify(reloadExpected.raw)}&&JSON.stringify(decisionFilterLocks.decisionOrgan)===${JSON.stringify(reloadExpected.decision)}&&JSON.stringify(decisionActivityFilters.type)===${JSON.stringify(reloadExpected.activity)}&&rawFilterMatchMode===${JSON.stringify(reloadExpected.rawMode)}&&decisionFilterMatchMode===${JSON.stringify(reloadExpected.decisionMode)}&&decisionActivityFilterMatchMode===${JSON.stringify(reloadExpected.activityMode)}&&JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])]))===${JSON.stringify(reloadExpected.calculator)}`,500);
