...
try{
  ...
  await waitFor(`!!rawDataPromise&&!!rawProgressiveSearchStateFinal&&!rawReady&&rawProgressiveSearchStateFinal.index>0`,400);
  await waitFor(`!rawReady&&document.getElementById('rawYear').options.length>1&&document.getElementById('rawParty').options.length>1`,400);
  ...
  const rawBackgroundLoad=await evaluate(`(async()=>{
    const load=rawDataPromise,state=rawProgressiveSearchStateFinal,startIndex=state?.index||0;
    await setTopView('calculator');
    await new Promise(resolve=>setTimeout(resolve,250));
    const awayIndex=state?.index||0,awayReady=rawReady;
    const sameLoadWhileAway=awayReady||rawDataPromise===load,sameStateWhileAway=awayReady||rawProgressiveSearchStateFinal===state;
    await setTopView('raw');
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    return {startedInProgress:!!load&&!awayReady,startIndex,awayIndex,continuedWhileAway:awayReady||awayIndex>=startIndex,sameLoadWhileAway,sameStateWhileAway,sameLoadOnReturn:rawReady||rawDataPromise===load,sameStateOnReturn:rawReady||rawProgressiveSearchStateFinal===state,view:currentTopView()};
  })()`);
