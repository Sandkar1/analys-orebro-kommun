...
try{
  ...
  const rawFilterClearRestore=await evaluate(`(async()=>{
    ...
    const singleToggle=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]'),singleGroupHint=singleToggle?.querySelector('small')?.textContent.trim()||'',modeBefore=rawFilterMatchMode;
    singleToggle?.click();
    ...
    return {year,fullCount,filteredCount,firstFilterSettled,baselinePinned:rawBaselineResultMemoFinal.has(baselineKey),singleGroupHint,singleModeImmediate,firstRestore,secondFilter,secondRestore};
  })()`);
