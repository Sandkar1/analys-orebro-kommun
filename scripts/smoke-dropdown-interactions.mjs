...
try{
  ...
  const rawFilterClearRestore=await evaluate(`(async()=>{
    const waitCurrent=async()=>{for(let attempt=0;attempt<800;attempt++){if(rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal()&&!progressiveSearchJobsFinal.has('raw'))return true;await new Promise(resolve=>setTimeout(resolve,10));}return false;};
    const originalMode=rawFilterMatchMode,fullCount=rawProgressiveSearchStateFinal.matches.length,baselineKey=rawResultMemoKeyFinal(),year=rawFilterOptionsFinal()?.years?.[0]||'';
    rawFilterLocks.rawYear=year?[String(year)]:[];buildRawFilters();rawScheduleProgressiveFinal();
    const firstFilterSettled=await waitCurrent(),filteredCount=rawProgressiveSearchStateFinal.matches.length;
    renderRawFilterLocks();
    const singleToggle=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]'),visibleHelperRemoved=!singleToggle?.querySelector('small'),singleFilterCount=singleToggle?.dataset.activeFilters==='1',modeBefore=rawFilterMatchMode;
    singleToggle?.click();
    const singleModeImmediate={changed:rawFilterMatchMode!==modeBefore,noJob:!progressiveSearchJobsFinal.has('raw'),complete:!!rawProgressiveSearchStateFinal?.complete,count:rawProgressiveSearchStateFinal?.matches.length||0};
    rawFilterLocks.rawYear=[];buildRawFilters();const firstRestoreStarted=performance.now();rawScheduleProgressiveFinal();
    const firstRestore={ms:performance.now()-firstRestoreStarted,noJob:!progressiveSearchJobsFinal.has('raw'),complete:!!rawProgressiveSearchStateFinal?.complete,count:rawProgressiveSearchStateFinal?.matches.length||0};
    rawFilterLocks.rawYear=year?[String(year)]:[];buildRawFilters();const secondFilterStarted=performance.now();rawScheduleProgressiveFinal();
    const secondFilter={ms:performance.now()-secondFilterStarted,noJob:!progressiveSearchJobsFinal.has('raw'),complete:!!rawProgressiveSearchStateFinal?.complete,count:rawProgressiveSearchStateFinal?.matches.length||0};
    rawFilterLocks.rawYear=[];buildRawFilters();const secondRestoreStarted=performance.now();rawScheduleProgressiveFinal();
    const secondRestore={ms:performance.now()-secondRestoreStarted,noJob:!progressiveSearchJobsFinal.has('raw'),complete:!!rawProgressiveSearchStateFinal?.complete,count:rawProgressiveSearchStateFinal?.matches.length||0};
    rawFilterMatchMode=originalMode;renderRawFilterLocks();rawScheduleProgressiveFinal();
    return {year,fullCount,filteredCount,firstFilterSettled,baselinePinned:rawBaselineResultMemoFinal.has(baselineKey),visibleHelperRemoved,singleFilterCount,singleModeImmediate,firstRestore,secondFilter,secondRestore};
  })()`);
  ...
  const decisionOverviewResume=await evaluate(`(async()=>{
    ...
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    const normalRows=document.getElementById('decisionBody').children.length;
    await setTopView('calculator');
    decisionStableListRenderFinal={key:decisionStableListKeyFinal(),progressiveState:{stale:true},filteredRows:[],rows:[],rendered:0};
    document.getElementById('decisionBody').replaceChildren();
    const stateBeforeReturn=decisionProgressiveSearchStateFinal;
    await setTopView('decision');
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    return {expected,actual:overview.querySelector('b')?.textContent||'',busy:overview.getAttribute('aria-busy'),view:currentTopView(),normalRows,recoveredRows:document.getElementById('decisionBody').children.length,sameResultState:stateBeforeReturn===decisionProgressiveSearchStateFinal,noRescan:!progressiveSearchJobsFinal.has('decision')};
  })()`);
  ...
  await evaluate(`(()=>{rawFilterMatchMode='or';rawFilterLocks.rawYear=selectedRawValues('rawYear').slice(0,1);rawFilterLocks.rawParty=['S'];buildRawFilters();rawScheduleProgressiveFinal();return true})()`);
  ...
  const rawFilterModeBefore=await evaluate(`(()=>{const toggle=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]'),filters={mode:rawFilterMatchMode,years:selectedRawValues('rawYear'),elections:selectedRawValues('rawElection'),counties:selectedRawValues('rawCounty'),municipalities:selectedRawValues('rawMunicipality'),parties:selectedRawValues('rawParty')},query=fuzzySearchNormalize(document.getElementById('rawSearch').value);return {mode:rawFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),afterClear:!!document.querySelector('#rawFilterLocks [data-clear-all-filters]+[data-filter-mode-toggle]'),matches:rawProgressiveSearchStateFinal?.matches.length||0,expected:rawRows.filter(row=>rawProgressivePredicateFinal(row,filters,query)).length,locks:Object.fromEntries(Object.entries(rawFilterLocks).map(([key,value])=>[key,[...value]])),unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  ...
  const rawFilterModeAfter=await evaluate(`(()=>{const toggle=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]'),filters={mode:rawFilterMatchMode,years:selectedRawValues('rawYear'),elections:selectedRawValues('rawElection'),counties:selectedRawValues('rawCounty'),municipalities:selectedRawValues('rawMunicipality'),parties:selectedRawValues('rawParty')},query=fuzzySearchNormalize(document.getElementById('rawSearch').value);return {mode:rawFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),pressed:toggle?.getAttribute('aria-pressed'),matches:rawProgressiveSearchStateFinal?.matches.length||0,expected:rawRows.filter(row=>rawProgressivePredicateFinal(row,filters,query)).length,locks:Object.fromEntries(Object.entries(rawFilterLocks).map(([key,value])=>[key,[...value]])),unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  await evaluate(`(()=>{Object.keys(rawFilterLocks).forEach(key=>{rawFilterLocks[key]=[];});rawFilterMatchMode='or';rawFilterLocks.rawYear=rawFilterOptionsFinal().years.slice(0,2).map(String);buildRawFilters();rawScheduleProgressiveFinal();return true})()`);
  await waitFor(`rawFilterMatchMode==='or'&&rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal()`,500);
  const sameCategoryUnion=await evaluate(`({mode:rawFilterMatchMode,values:[...rawFilterLocks.rawYear],matches:rawProgressiveSearchStateFinal.matches.length,expected:rawRows.filter(row=>rawProgressivePredicateFinal(row,{mode:'or',years:selectedRawValues('rawYear'),elections:[],counties:[],municipalities:[],parties:[]},'')).length})`);
  await evaluate(`document.querySelector('#rawFilterLocks [data-filter-mode-toggle]').click()`);
  await waitFor(`rawFilterMatchMode==='and'&&rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal()`,500);
  const sameCategoryIntersection=await evaluate(`({mode:rawFilterMatchMode,values:[...rawFilterLocks.rawYear],matches:rawProgressiveSearchStateFinal.matches.length,expected:rawRows.filter(row=>rawProgressivePredicateFinal(row,{mode:'and',years:selectedRawValues('rawYear'),elections:[],counties:[],municipalities:[],parties:[]},'')).length})`);
  const sameCategoryFilterModes={union:sameCategoryUnion,intersection:sameCategoryIntersection};
  ...
  const decisionFilterModeBefore=await evaluate(`(()=>{const toggle=document.querySelector('#decisionFilterLocks [data-filter-mode-toggle]');return {mode:decisionFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),afterClear:!!document.querySelector('#decisionFilterLocks [data-clear-all-filters]+[data-filter-mode-toggle]'),matches:decisionProgressiveSearchStateFinal?.filteredMatches.length||0,unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  ...
  const decisionFilterModeAfter=await evaluate(`(()=>{const toggle=document.querySelector('#decisionFilterLocks [data-filter-mode-toggle]');return {mode:decisionFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),pressed:toggle?.getAttribute('aria-pressed'),matches:decisionProgressiveSearchStateFinal?.filteredMatches.length||0,unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  ...
  const activityFilterModeBefore=await evaluate(`(()=>{const toggle=document.querySelector('#decisionActivityFilterLocks [data-filter-mode-toggle]');return {mode:decisionActivityFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),afterClear:!!document.querySelector('#decisionActivityFilterLocks [data-clear-all-filters]+[data-filter-mode-toggle]'),matches:decisionActivityProgressiveSearchStateFinal?.matches.length||0,expected:filteredDecisionActivityRowsBeforeProgressiveFinal().length,unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  ...
  const activityFilterModeAfter=await evaluate(`(()=>{const toggle=document.querySelector('#decisionActivityFilterLocks [data-filter-mode-toggle]');return {mode:decisionActivityFilterMatchMode,label:toggle?.querySelector('.filter-mode-label')?.textContent.trim(),pressed:toggle?.getAttribute('aria-pressed'),matches:decisionActivityProgressiveSearchStateFinal?.matches.length||0,expected:filteredDecisionActivityRowsBeforeProgressiveFinal().length,unionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-union-fill')).opacity),intersectionOpacity:Number(getComputedStyle(toggle.querySelector('.venn-intersection-fill')).opacity)};})()`);
  ...
  const filterSemantics=await evaluate(`({sameCategoryUnion:filterValueSetsMatch([{values:['2010','2014'],matches:value=>value==='2014'}],'or'),sameCategoryIntersection:filterValueSetsMatch([{values:['2010','2014'],matches:value=>value==='2014'}],'and'),crossCategoryUnion:filterValueSetsMatch([{values:['2014'],matches:value=>value==='2014'},{values:['S'],matches:value=>value==='M'}],'or'),crossCategoryIntersection:filterValueSetsMatch([{values:['2014'],matches:value=>value==='2014'},{values:['S'],matches:value=>value==='M'}],'and'),empty:filterValueSetsMatch([],'and')})`);
  await evaluate(`(()=>{Object.keys(rawFilterLocks).forEach(key=>{rawFilterLocks[key]=[];});rawFilterLocks.rawYear=['2010'];rawFilterLocks.rawParty=['S'];rawFilterMatchMode='and';buildRawFilters();rawScheduleProgressiveFinal();return true})()`);
  await waitFor(`rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal()`,500);
  const reloadExpected=await evaluate(`({top:currentTopView(),raw:JSON.stringify(rawFilterLocks.rawYear),rawParty:JSON.stringify(rawFilterLocks.rawParty),decision:JSON.stringify(decisionFilterLocks.decisionOrgan),activity:JSON.stringify(decisionActivityFilters.type),rawMode:rawFilterMatchMode,decisionMode:decisionFilterMatchMode,activityMode:decisionActivityFilterMatchMode,calculator:JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])]))})`);
  const reloadHash=await evaluate(`(async()=>{await updateUrlHashSession();return location.hash})()`);
  await cdp.send('Page.enable');
  await cdp.send('Page.reload',{ignoreCache:true});
  await waitFor(`document.readyState==='complete'&&typeof currentTopView==='function'&&!!rawDataPromise&&!rawReady&&JSON.stringify(rawFilterLocks.rawYear)===${JSON.stringify(reloadExpected.raw)}&&JSON.stringify(rawFilterLocks.rawParty)===${JSON.stringify(reloadExpected.rawParty)}&&rawFilterMatchMode===${JSON.stringify(reloadExpected.rawMode)}&&rawProgressiveSearchStateFinal?.index>0`,500);
  const loadingFilterContinuity=await evaluate(`(async()=>{
    const load=rawDataPromise,frame=()=>new Promise(resolve=>requestAnimationFrame(resolve));await setTopView('raw');
    const restoredState=rawProgressiveSearchStateFinal,restoredStart={index:restoredState.index,matches:restoredState.matches.length,status:document.getElementById('rawCount').textContent};
    const restoredStatuses=new Set([restoredStart.status]);let restoredMaxGap=0,last=performance.now();
    for(let attempt=0;attempt<18&&!rawReady;attempt++){await frame();const now=performance.now();restoredMaxGap=Math.max(restoredMaxGap,now-last);last=now;restoredStatuses.add(document.getElementById('rawCount').textContent);}
    const restoredEnd={index:restoredState.index,matches:restoredState.matches.length,sameState:rawProgressiveSearchStateFinal===restoredState,current:restoredState.key===rawProgressiveSearchKeyFinal(),statuses:restoredStatuses.size};
    rawFilterLocks.rawElection=['municipal'];buildRawFilters();rawScheduleProgressiveFinal();await frame();await frame();
    const retargetedState=rawProgressiveSearchStateFinal,retargetStart={index:retargetedState.index,matches:retargetedState.matches.length,ready:rawReady,complete:!!retargetedState.complete};
    const retargetStatuses=new Set([document.getElementById('rawCount').textContent]);let retargetMaxGap=0;last=performance.now();
    for(let attempt=0;attempt<90&&!rawReady;attempt++){await frame();const now=performance.now();retargetMaxGap=Math.max(retargetMaxGap,now-last);last=now;retargetStatuses.add(document.getElementById('rawCount').textContent);}
    const retargetEnd={index:retargetedState.index,matches:retargetedState.matches.length,sameState:rawProgressiveSearchStateFinal===retargetedState,current:retargetedState.key===rawProgressiveSearchKeyFinal(),statuses:retargetStatuses.size};
    await load;for(let attempt=0;attempt<500&&!(rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal());attempt++)await new Promise(resolve=>setTimeout(resolve,10));
    const filters={mode:rawFilterMatchMode,years:selectedRawValues('rawYear'),elections:selectedRawValues('rawElection'),counties:selectedRawValues('rawCounty'),municipalities:selectedRawValues('rawMunicipality'),parties:selectedRawValues('rawParty')},query=fuzzySearchNormalize(document.getElementById('rawSearch').value),finalState=rawProgressiveSearchStateFinal;
    const final={ready:rawReady,complete:!!finalState?.complete,matches:finalState?.matches.length||0,expected:rawRows.filter(row=>rawProgressivePredicateFinal(row,filters,query)).length,locks:{year:[...rawFilterLocks.rawYear],party:[...rawFilterLocks.rawParty],election:[...rawFilterLocks.rawElection]},sameLoad:load===rawDataPromise||rawDataPromise===null,noSeparateJob:!progressiveSearchJobsFinal.has('raw')};
    await setTopView(${JSON.stringify(reloadExpected.top)});return {restoredStart,restoredEnd,restoredMaxGap,retargetStart,retargetEnd,retargetMaxGap,retargeted:retargetedState!==restoredState,retargetStatuses:retargetStatuses.size,final};
  })()`);
  await waitFor(`currentTopView()===${JSON.stringify(reloadExpected.top)}&&!urlHashApplying&&JSON.stringify(decisionFilterLocks.decisionOrgan)===${JSON.stringify(reloadExpected.decision)}&&JSON.stringify(decisionActivityFilters.type)===${JSON.stringify(reloadExpected.activity)}&&decisionFilterMatchMode===${JSON.stringify(reloadExpected.decisionMode)}&&decisionActivityFilterMatchMode===${JSON.stringify(reloadExpected.activityMode)}&&document.getElementById('decisionActivityType').options.length>1&&JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])]))===${JSON.stringify(reloadExpected.calculator)}`,500);
  const reload=await evaluate(`({mode:location.protocol==='file:'?'file':'http',hash:location.hash,top:currentTopView(),raw:JSON.stringify(rawFilterLocks.rawYear),decision:JSON.stringify(decisionFilterLocks.decisionOrgan),activity:JSON.stringify(decisionActivityFilters.type),rawMode:rawFilterMatchMode,decisionMode:decisionFilterMatchMode,activityMode:decisionActivityFilterMatchMode,buttonModes:{raw:document.querySelector('#rawFilterLocks [data-filter-mode-toggle]')?.dataset.mode,decision:document.querySelector('#decisionFilterLocks [data-filter-mode-toggle]')?.dataset.mode,activity:document.querySelector('#decisionActivityFilterLocks [data-filter-mode-toggle]')?.dataset.mode},calculator:JSON.stringify(tabs.map(tab=>[tab.role,tab.title,tab.dirty,tab.parties.map(party=>[party.name,party.votes,party.icon])])),activityOptions:document.getElementById('decisionActivityType').options.length,promptDisabled:document.getElementById('decisionActivityType').options[0]?.disabled??true})`);
  ...
  const result={mode:fileAudit?'file':'http',functionalFailures,eagerStartup,calculatorTemplate,calculatorImport,calculator,calculatorIcon,rawProgressiveFilters,rawBackgroundLoad,rawFilterClearRestore,rawSortReuse,rawPartyCanonical,rawPartyOrdering,inventory,functional,raw,decisionCold,decisionColdFilterResults,decisionMemberMarkers,searchableDropdown,searchableDropdownPick,decisionOverviewResume,calendar:{before:calendarBefore,monthAfter:calendarMonthAfter,yearAfter:calendarYearAfter},navigationDuringDecisionLoad,navigationDuringActivityLoad,decision,activity,additive,filterModes,sameCategoryFilterModes,filterSemantics,loadingFilterContinuity,reloadExpected,reloadHashChanged:!!reloadHash,reload,postReload};
  console.log(JSON.stringify(filterResultsOnly?{eagerStartup,calculatorTemplate,rawProgressiveFilters,rawBackgroundLoad,rawFilterClearRestore,rawSortReuse,rawPartyCanonical,decisionColdFilterResults,decisionMemberMarkers,searchableDropdown,searchableDropdownPick,decisionOverviewResume,filterModes,sameCategoryFilterModes,filterSemantics,loadingFilterContinuity,reload}:result,null,2));
  ...
  if(!rawFilterClearRestore.year||!rawFilterClearRestore.firstFilterSettled||!rawFilterClearRestore.baselinePinned||!rawFilterClearRestore.visibleHelperRemoved||!rawFilterClearRestore.singleFilterCount||!rawFilterClearRestore.singleModeImmediate.changed||!rawFilterClearRestore.singleModeImmediate.noJob||!rawFilterClearRestore.singleModeImmediate.complete||rawFilterClearRestore.singleModeImmediate.count!==rawFilterClearRestore.filteredCount||!rawFilterClearRestore.firstRestore.noJob||!rawFilterClearRestore.firstRestore.complete||rawFilterClearRestore.firstRestore.count!==rawFilterClearRestore.fullCount||!rawFilterClearRestore.secondFilter.noJob||!rawFilterClearRestore.secondFilter.complete||rawFilterClearRestore.secondFilter.count!==rawFilterClearRestore.filteredCount||!rawFilterClearRestore.secondRestore.noJob||!rawFilterClearRestore.secondRestore.complete||rawFilterClearRestore.secondRestore.count!==rawFilterClearRestore.fullCount)process.exitCode=1;
  ...
  if(!filterSemantics.sameCategoryUnion||filterSemantics.sameCategoryIntersection||!filterSemantics.crossCategoryUnion||filterSemantics.crossCategoryIntersection||!filterSemantics.empty)process.exitCode=1;
  if(sameCategoryFilterModes.union.mode!=='or'||sameCategoryFilterModes.union.values.length!==2||sameCategoryFilterModes.union.matches<=0||sameCategoryFilterModes.union.matches!==sameCategoryFilterModes.union.expected||sameCategoryFilterModes.intersection.mode!=='and'||sameCategoryFilterModes.intersection.values.length!==2||sameCategoryFilterModes.intersection.matches!==0||sameCategoryFilterModes.intersection.matches!==sameCategoryFilterModes.intersection.expected)process.exitCode=1;
  if(Object.values(filterModes).some(item=>item.before.mode!=='or'||item.before.label!=='OR'||!item.before.afterClear||item.before.matches<=0||item.before.unionOpacity<=0||item.before.intersectionOpacity!==0||item.after.mode!=='and'||item.after.label!=='AND'||item.after.pressed!=='true'||item.after.matches<0||item.after.unionOpacity!==0||item.after.intersectionOpacity<=0))process.exitCode=1;
  ...
  if(filterModes.decision.before.matches<=filterModes.decision.after.matches||filterModes.activity.before.matches<=filterModes.activity.after.matches)process.exitCode=1;
  if(loadingFilterContinuity.restoredEnd.index<=loadingFilterContinuity.restoredStart.index||loadingFilterContinuity.restoredEnd.matches<loadingFilterContinuity.restoredStart.matches||!loadingFilterContinuity.restoredEnd.sameState||!loadingFilterContinuity.restoredEnd.current||loadingFilterContinuity.restoredEnd.statuses<2||loadingFilterContinuity.restoredMaxGap>500||!loadingFilterContinuity.retargeted||loadingFilterContinuity.retargetStart.ready||loadingFilterContinuity.retargetStart.complete||loadingFilterContinuity.retargetEnd.index<=loadingFilterContinuity.retargetStart.index||loadingFilterContinuity.retargetEnd.matches<loadingFilterContinuity.retargetStart.matches||!loadingFilterContinuity.retargetEnd.sameState||!loadingFilterContinuity.retargetEnd.current||loadingFilterContinuity.retargetStatuses<2||loadingFilterContinuity.retargetMaxGap>500||!loadingFilterContinuity.final.ready||!loadingFilterContinuity.final.complete||loadingFilterContinuity.final.matches!==loadingFilterContinuity.final.expected||JSON.stringify(loadingFilterContinuity.final.locks)!==JSON.stringify({year:['2010'],party:['S'],election:['municipal']})||!loadingFilterContinuity.final.sameLoad||!loadingFilterContinuity.final.noSeparateJob)process.exitCode=1;
