...
try{
  ...
  const mobileFilterModes=await evaluate(`(async()=>{
    ...
    const buttonAudit=button=>{button.scrollIntoView({block:'center',inline:'nearest'});const rect=button.getBoundingClientRect(),hit=document.elementFromPoint(rect.left+rect.width/2,rect.top+rect.height/2);return {hit:hit===button||button.contains(hit),width:rect.width,height:rect.height,mode:button.dataset.mode,label:button.querySelector('.filter-mode-label')?.textContent.trim(),union:Number(getComputedStyle(button.querySelector('.venn-union-fill')).opacity),intersection:Number(getComputedStyle(button.querySelector('.venn-intersection-fill')).opacity)};};
    await setTopView('raw');Object.keys(rawFilterLocks).forEach(key=>{rawFilterLocks[key]=[];});rawFilterLocks.rawYear=[String(rawFilterOptionsFinal().years[0])];rawFilterLocks.rawParty=['S'];rawFilterMatchMode='or';buildRawFilters();rawScheduleProgressiveFinal();await waitUntil(()=>rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal());
    let button=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]'),rawBefore={...buttonAudit(button),matches:rawProgressiveSearchStateFinal.matches.length};button.click();await waitUntil(()=>rawFilterMatchMode==='and'&&rawProgressiveSearchStateFinal?.complete&&rawProgressiveSearchStateFinal.key===rawProgressiveSearchKeyFinal());button=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]');const rawAfter={...buttonAudit(button),matches:rawProgressiveSearchStateFinal.matches.length};
    await setTopView('decision');const organs=[...document.getElementById('decisionOrgan').options].map(option=>option.value).filter(value=>value&&!value.startsWith('__')).slice(0,2);decisionFilterIds.forEach(id=>{decisionFilterLocks[id]=[];});decisionFilterLocks.decisionOrgan=organs;decisionFilterMatchMode='or';buildDecisionFilters();renderDecisionFilterLocks();decisionScheduleProgressiveRefreshFinal();await waitUntil(()=>decisionProgressiveStateIsCurrentFinal());button=document.querySelector('#decisionFilterLocks [data-filter-mode-toggle]');const decisionBefore={...buttonAudit(button),matches:decisionProgressiveSearchStateFinal.filteredMatches.length,values:organs.length};button.click();await waitUntil(()=>decisionFilterMatchMode==='and'&&decisionProgressiveStateIsCurrentFinal());button=document.querySelector('#decisionFilterLocks [data-filter-mode-toggle]');const decisionAfter={...buttonAudit(button),matches:decisionProgressiveSearchStateFinal.filteredMatches.length};
    await setTopView('decisionActivity');const types=[...document.getElementById('decisionActivityType').options].map(option=>option.value).filter(value=>value&&!value.startsWith('__')).slice(0,2);Object.keys(decisionActivityFilters).forEach(key=>{decisionActivityFilters[key]=[];});decisionActivityFilters.type=types;decisionActivityFilterMatchMode='or';buildDecisionActivityFilters();renderActivityFilterLocks();renderDecisionActivityView();await waitUntil(()=>decisionActivityProgressiveSearchStateFinal?.complete&&decisionActivityProgressiveSearchStateFinal.key===decisionActivityProgressiveSearchKeyFinal());button=document.querySelector('#decisionActivityFilterLocks [data-filter-mode-toggle]');const activityBefore={...buttonAudit(button),matches:decisionActivityProgressiveSearchStateFinal.matches.length,values:types.length};button.click();await waitUntil(()=>decisionActivityFilterMatchMode==='and'&&decisionActivityProgressiveSearchStateFinal?.complete&&decisionActivityProgressiveSearchStateFinal.key===decisionActivityProgressiveSearchKeyFinal());button=document.querySelector('#decisionActivityFilterLocks [data-filter-mode-toggle]');const activityAfter={...buttonAudit(button),matches:decisionActivityProgressiveSearchStateFinal.matches.length};
    return {raw:{before:rawBefore,after:rawAfter},decision:{before:decisionBefore,after:decisionAfter},activity:{before:activityBefore,after:activityAfter}};
  })()`);
  ...
  const zoomToggle=await evaluate(`(async()=>{await setTopView('raw');const toggle=document.querySelector('#rawFilterLocks [data-filter-mode-toggle]');toggle.scrollIntoView({block:'center',inline:'nearest'});await new Promise(resolve=>requestAnimationFrame(resolve));const rect=toggle.getBoundingClientRect(),hit=document.elementFromPoint(rect.left+rect.width/2,rect.top+rect.height/2);return {hit:hit===toggle||toggle.contains(hit),width:rect.width,height:rect.height};})()`);
  ...
  Object.assign(zoomAudit,{toggleHit:zoomToggle.hit,toggleWidth:zoomToggle.width,toggleHeight:zoomToggle.height});
  ...
  const output={failures,calculatorScroll,searchableDropdownRect,decisionSwitchStability,decisionLayout,calendarRect,dialogRect,mobileFilterModes,zoomAudit,desktopWrapper,results};
  console.log(JSON.stringify(output,null,2));
  const filterModeFailure=Object.values(mobileFilterModes).some(item=>!item.before.hit||item.before.width<44||item.before.height<24||item.before.mode!=='or'||item.before.label!=='OR'||item.before.union<=0||item.before.intersection!==0||!item.after.hit||item.after.mode!=='and'||item.after.label!=='AND'||item.after.union!==0||item.after.intersection<=0||item.before.matches<=item.after.matches);
  if(failures.length||!calculatorScroll.inputScrollable||!calculatorScroll.resultScrollable||searchableDropdownRect.left<0||searchableDropdownRect.right>searchableDropdownRect.viewportWidth+1||searchableDropdownRect.top<0||searchableDropdownRect.bottom>searchableDropdownRect.viewportHeight+1||searchableDropdownRect.inputFont<16||searchableDropdownRect.results<=0||decisionSwitchStability.originalRows<=0||decisionSwitchStability.cycles.some(cycle=>!cycle.sameState||!cycle.current||cycle.job||!cycle.finished||cycle.rows<=0)||calendarRect.left<0||calendarRect.right>calendarRect.viewport+1||dialogRect.top<0||dialogRect.bottom>dialogRect.viewportHeight+1||filterModeFailure||mobileFilterModes.decision.before.values!==2||mobileFilterModes.activity.before.values!==2||zoomAudit.scale<1.9||zoomAudit.pageScrollWidth>zoomAudit.layoutWidth+1||zoomAudit.bodyScrollWidth>zoomAudit.layoutWidth+1||!zoomAudit.panelWithinLayout||!zoomAudit.toggleHit||zoomAudit.toggleWidth<44||zoomAudit.toggleHeight<24||desktopWrapper!=='contents')process.exitCode=1;
}finally{
