...
function rawComparable(row,col){const text=rawValue(row,col);if(col==='county_name')return normalizeRegionName(text);return text;}
function rawDisplay(col,value){const text=rawValue({[col]:value},col);if(col==='election_type')return rawElectionLabels[text]||text;if(col==='county_name')return normalizeRegionName(text);return text;}
...
function selectedRawValues(id){return rawFilterLocks[id]||[];}
function selectedRawLabels(id){const sel=$(id),col=sel?.dataset.col;return selectedRawValues(id).map(v=>col?rawDisplay(col,v):v);}
function setSelectOptions(id,values,selected=[],col='',allLabel='Alla'){const sel=$(id);rawFilterLocks[id]=normalizeRawSelectionState(selected.length?selected:rawFilterLocks[id]).filter(v=>values.map(String).includes(String(v)));const locked=new Set(rawFilterLocks[id].map(String));const allOption=locked.size?`<option value="" disabled selected>Välj fler...</option><option value="${rawFilterClearValue}">${esc(allLabel)}</option>`:`<option value="">${esc(allLabel)}</option>`;const options=[allOption,...values.filter(v=>!locked.has(String(v))).map(v=>`<option value="${esc(v)}">${esc(col?rawDisplay(col,v):v)}</option>`)].join('');sel.dataset.col=col;sel.innerHTML=options;sel.value='';}
function renderRawFilterLocks(){rawFilterIds.forEach(id=>{const sel=$(id);if(!sel)return;let lock=sel.parentElement.querySelector('.raw-filter-lock');const values=selectedRawValues(id);if(!values.length){if(lock)lock.remove();return;}if(!lock){lock=document.createElement('div');lock.className='raw-filter-lock';sel.insertAdjacentElement('afterend',lock);}const col=sel.dataset.col;lock.innerHTML=values.map(v=>`<span class="raw-filter-chip"><span>${esc(col?rawDisplay(col,v):v)}</span><button type="button" data-value="${esc(v)}" title="Rensa låst filter" aria-label="Rensa låst filter">×</button></span>`).join('');lock.querySelectorAll('button').forEach(btn=>btn.onclick=()=>{rawFilterLocks[id]=selectedRawValues(id).filter(v=>v!==btn.dataset.value);handleRawFilterChange(id);});});}
function buildRawFilters(){const selectedCounties=selectedRawValues('rawCounty');const selectedMunicipalities=selectedRawValues('rawMunicipality');setSelectOptions('rawYear',uniqueRaw('year'),rawFilterLocks.rawYear,'year');setSelectOptions('rawElection',uniqueRawElectionTypes(),rawFilterLocks.rawElection,'election_type');setSelectOptions('rawCounty',uniqueRaw('county_name'),rawFilterLocks.rawCounty,'county_name');const countyRows=selectedCounties.length?rawRows.filter(r=>selectedCounties.includes(rawComparable(r,'county_name'))):rawRows;setSelectOptions('rawMunicipality',uniqueRawFromRows(countyRows,'municipality_name'),rawFilterLocks.rawMunicipality,'municipality_name');const municipalityRows=(selectedCounties.length||selectedMunicipalities.length)?rawRows.filter(r=>(!selectedCounties.length||selectedCounties.includes(rawComparable(r,'county_name')))&&(!selectedMunicipalities.length||selectedMunicipalities.includes(rawComparable(r,'municipality_name')))):rawRows;setSelectOptions('rawParty',uniqueRawPartiesByVotes(rawRows),rawFilterLocks.rawParty,'party_standard');renderRawFilterLocks();}
function handleRawFilterChange(id){const sel=$(id),value=sel?.value;if(value===rawFilterClearValue)rawFilterLocks[id]=[];else if(value){if(!selectedRawValues(id).includes(value))rawFilterLocks[id]=[...selectedRawValues(id),value];}else rawFilterLocks[id]=[];if(id==='rawCounty')rawFilterLocks.rawMunicipality=rawFilterLocks.rawMunicipality.filter(v=>uniqueRawFromRows(rawFilterLocks.rawCounty.length?rawRows.filter(r=>rawFilterLocks.rawCounty.includes(rawComparable(r,'county_name'))):rawRows,'municipality_name').includes(v));buildRawFilters();resetRawPage();renderRawTable();}
...
function syncRawSelectionUi(){const total=rawLastEligibleCount+rawLastIneligibleCount;const countText=`Totalt: ${fmtInt(total)}. Giltiga rader: ${fmtInt(rawLastEligibleCount)}. Ogiltiga rader: ${fmtInt(rawLastIneligibleCount)}. Valda rader: ${fmtInt(selectedRawCount())}.`;$('rawCount').textContent=countText;const body=$('rawEligibleBody');if(!body)return;body.querySelectorAll('.raw-selectable-row').forEach(row=>{const id=Number(row.dataset.id);const selected=rawSelected.has(id);row.classList.toggle('selected',selected);const box=row.querySelector('.raw-pick');if(box)box.checked=selected;});}
