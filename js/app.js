...
function cleanTabState(t){const snapshot=compactSnapshot(expandSnapshot(t.snapshot||buildResultSnapshot(t)));return {i:t.id,r:isTemplateTab(t)?'template':'calculation',t:t.title,m:t.method,f:t.factor,s:t.seats,ms:t.memberSeats,ss:t.substituteSeats,sd:t.seed,stv:t.sourceTotalVotes??null,vm:calcVoteInputMode(t.voteInputMode),d:isTemplateTab(t)?false:!!t.dirty,p:(t.parties||[]).map(p=>[p.id,p.name,Number(p.votes)||0,normalizeCalcIcon(p.icon)]),sn:snapshot};}
