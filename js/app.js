...
$('calculate').onclick=calculateCurrent;$('reroll').onclick=()=>{if(isTemplateTab())return;readInputs(false);current().seed=newSeed();calculateCurrent();};$('add').onclick=()=>{readInputs(true);const t=current();t.parties.push({id:Date.now(),name:'Nytt parti eller grupp',votes:0,icon:15,order:t.parties.length});renderAll();showNotice(isTemplateTab()?'Huvudvyn har uppdaterats. Nya beräkningsflikar kommer att kopiera den här partimallen.':'Ändringar väntar. Tryck Beräkna mandat för att uppdatera resultatet.');};$('reset').onclick=async()=>{if(await openResetConfirm())resetTemplateInitialData();};
