...
function cleanTabState(t){const snapshot=compactSnapshot(expandSnapshot(t.snapshot||buildResultSnapshot(t)));return {i:t.id,r:isTemplateTab(t)?'template':'calculation',t:t.title,m:t.method,f:t.factor,s:t.seats,ms:t.memberSeats,ss:t.substituteSeats,sd:t.seed,stv:t.sourceTotalVotes??null,vm:calcVoteInputMode(t.voteInputMode),vb:Math.max(1,Math.round(Number(t.voteCountBasis)||100)),vd:normalizeCalcVoteModeDrafts(t.voteModeDrafts),d:isTemplateTab(t)?false:!!t.dirty,p:(t.parties||[]).map(p=>[p.id,p.name,Number(p.votes)||0,normalizeCalcIcon(p.icon)]),sn:snapshot};}
...
function normalizeImportedPayload(payload){if(payload?.f==='m'&&Array.isArray(payload.t))return {activeTab:Number(payload.a)||0,topView:payload.tv==='raw'?'raw':['decision','decisionActivity'].includes(payload.tv)&&municipalWorkEnabled?payload.tv:'calculator',rawState:payload.rv||null,decisionState:payload.dv||null,tabs:payload.t.map((tab,i)=>({id:Number(tab.i)||0,role:tab.r==='template'?'template':'calculation',title:tab.t||`Beräkning ${i+1}`,method:tab.m||'dhondt',factor:tab.f||'1.2',seats:Number(tab.s)||17,memberSeats:Number(tab.ms??tab.s)||17,substituteSeats:Number(tab.ss)||11,seed:String(tab.sd||newSeed()),sourceTotalVotes:tab.stv??null,voteInputMode:calcVoteInputMode(tab.vm),voteCountBasis:Math.max(1,Math.round(Number(tab.vb)||100)),voteModeDrafts:normalizeCalcVoteModeDrafts(tab.vd),dirty:!!tab.d,parties:Array.isArray(tab.p)?tab.p.map((p,j)=>({id:Number(p?.[0])||Date.now()+j,name:p?.[1]||`Parti eller samverkansgrupp ${j+1}`,votes:Number(p?.[2])||0,icon:normalizeCalcIcon(p?.[3]),order:j})):[],snapshot:expandSnapshot(tab.sn)}))};if(payload?.format==='mandatfordelning-state'&&Array.isArray(payload.tabs))return {activeTab:Number(payload.activeTab)||0,topView:'calculator',rawState:null,decisionState:null,tabs:payload.tabs.map((tab,i)=>({...tab,role:tab.role==='template'?'template':'calculation',title:tab.title||`Beräkning ${i+1}`,voteInputMode:calcVoteInputMode(tab.voteInputMode),voteCountBasis:Math.max(1,Math.round(Number(tab.voteCountBasis)||100)),voteModeDrafts:normalizeCalcVoteModeDrafts(tab.voteModeDrafts),snapshot:expandSnapshot(tab.snapshot)}))};throw Error('Sessionen innehåller ingen giltig beräkningsdata.');}
...
function normalizeCalculatorTabStructure(importedTabs,importedActiveTab){const list=[...importedTabs];let selected=Math.max(0,Math.min(Number(importedActiveTab)||0,Math.max(0,list.length-1)));if(!list.length)list.push(makeTemplateTab(initialState));if(!isTemplateTab(list[0])){list.unshift(makeTemplateTab({...initialState,parties:list[0].parties}));selected++;}list[0].role='template';list[0].title='Huvudvy';list[0].result=null;list[0].dirty=false;list[0].snapshot=buildResultSnapshot(list[0]);for(let index=1;index<list.length;index++)list[index].role='calculation';return {tabs:list,activeTab:Math.max(0,Math.min(selected,list.length-1))};}
async function applyImportedState(payload){const normalized=normalizeImportedPayload(payload);if(!normalized.tabs.length)throw Error('Sessionen saknar flikar.');const structured=normalizeCalculatorTabStructure(normalized.tabs.map(hydrateImportedTab),normalized.activeTab);tabs=structured.tabs;nextTabId=tabs.reduce((max,t)=>Math.max(max,Number(t.id)||0),0)+1;activeTab=structured.activeTab;if(municipalWorkEnabled)applyDecisionState(normalized.decisionState);renderAll();await setTopView(normalized.topView||'calculator');await applyRawState(normalized.rawState);restoreUrlUiState(payload?.ui);}
...
$('calculate').onclick=calculateCurrent;$('reroll').onclick=()=>{if(isTemplateTab())return;readInputs(false);current().seed=newSeed();calculateCurrent();};$('add').onclick=()=>{readInputs(true);const t=current();t.parties.push({id:Date.now(),name:'Nytt parti eller grupp',votes:0,icon:15,order:t.parties.length});t.voteModeDrafts={};saveCalcVoteModeDraft(t);renderAll();showNotice(isTemplateTab()?'Huvudvyn har uppdaterats. Nya beräkningsflikar kommer att kopiera den här partimallen.':'Ändringar väntar. Tryck Beräkna mandat för att uppdatera resultatet.');};$('reset').onclick=async()=>{if(await openResetConfirm())resetTemplateInitialData();};
...
$('newTab').onclick=createCalculationFromTemplate;$('duplicateTab').onclick=duplicateCurrentCalculation;$('calcImportSource').onchange=syncCalculationImportButton;$('calcImportApply').onclick=importCalculationMandates;if($('closeTab'))$('closeTab').onclick=()=>closeCalcTab(activeTab);
...
$('decisionDecisionSearch').oninput=()=>{decisionSearchQuery=$('decisionDecisionSearch')?.value||'';decisionManualSortContextFinal='';persistDecisionSearchForReload(decisionSearchQuery);decisionActiveTab=0;resetDecisionPage();if(typeof decisionCancelProgressiveSearchForTypingFinal==='function')decisionCancelProgressiveSearchForTypingFinal();clearTimeout(decisionSearchInputTimer);decisionSearchInputTimer=setTimeout(()=>{decisionSearchInputTimer=null;scheduleTableSearch('decision','decisionDecisionSearch',['decisionBody'],()=>renderDecisionView());},100);};if($('decisionDecisionClear'))$('decisionDecisionClear').onclick=()=>{clearTimeout(decisionSearchInputTimer);decisionSearchInputTimer=null;decisionSearchQuery='';decisionManualSortContextFinal='';persistDecisionSearchForReload('');decisionActiveTab=0;resetDecisionPage();scheduleTableSearch('decision','decisionDecisionSearch',['decisionBody'],()=>renderDecisionView());};
...
async function initAppFromUrlHash(){
  ...
  if(!restored){
    tabs=[makeTemplateTab(initialState)];
    activeTab=0;
    renderAll();
    renderRawDraft();
...
$('calcImportApply').onclick=importCalculationMandatesAsCounts;
