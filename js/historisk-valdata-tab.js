...
function buildRawFilters(){const selectedCounties=selectedRawValues('rawCounty'),isOr=rawFilterMatchMode==='or';setSelectOptions('rawYear',uniqueRaw('year'),rawFilterLocks.rawYear,'year');setSelectOptions('rawElection',uniqueRawElectionTypes(),rawFilterLocks.rawElection,'election_type');setSelectOptions('rawCounty',uniqueRaw('county_name'),rawFilterLocks.rawCounty,'county_name');const countyRows=!isOr&&selectedCounties.length?rawRows.filter(r=>selectedCounties.includes(rawComparable(r,'county_name'))):rawRows;setSelectOptions('rawMunicipality',uniqueRawFromRows(countyRows,'municipality_name'),rawFilterLocks.rawMunicipality,'municipality_name');setSelectOptions('rawParty',isOr?uniqueRawPartiesByVotes(rawRows):rawPartyOptionsByCurrentContext(),rawFilterLocks.rawParty,'party_standard');renderRawFilterLocks();}
function handleRawFilterChange(id){const sel=$(id),value=sel?.value;if(value===rawFilterPromptValue)return;if(value===rawFilterClearValue)rawFilterLocks[id]=[];else if(value){if(!selectedRawValues(id).includes(value))rawFilterLocks[id]=[...selectedRawValues(id),value];}else rawFilterLocks[id]=[];if(rawFilterMatchMode!=='or'&&id==='rawCounty')rawFilterLocks.rawMunicipality=rawFilterLocks.rawMunicipality.filter(v=>uniqueRawFromRows(rawFilterLocks.rawCounty.length?rawRows.filter(r=>rawFilterLocks.rawCounty.includes(rawComparable(r,'county_name'))):rawRows,'municipality_name').includes(v));buildRawFilters();resetRawPage();renderRawTable();}
