...
function setSelectOptions(id,values,selected=[],col='',allLabel='Alla'){const sel=$(id);rawFilterLocks[id]=normalizeRawSelectionState(selected.length?selected:rawFilterLocks[id]).filter(v=>values.map(String).includes(String(v)));const locked=new Set(rawFilterLocks[id].map(String));const allOption=locked.size?`<option value="${rawFilterPromptValue}" selected>Välj fler...</option><option value="${rawFilterClearValue}">${esc(allLabel)}</option>`:`<option value="">${esc(allLabel)}</option>`;const options=[allOption,...values.filter(v=>!locked.has(String(v))).map(v=>`<option value="${esc(v)}">${esc(col?rawDisplay(col,v):v)}</option>`)].join('');sel.dataset.col=col;sel.innerHTML=options;sel.value=locked.size?rawFilterPromptValue:'';}
